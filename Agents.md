...
9. 外部アプリ終了後にホームへ戻ったケースでも、ホーム再表示後30秒タイマーを再開する。
10. 設定画面表示中は自動起動を実行しない。
11. 自動起動設定は SettingScreen から変更し、ViewModel→UseCase→Repository 経由で永続化する。
12. SettingScreen は「アプリ起動」「表示設定」の2タブ構成とする。
13. 表示設定タブのアプリ一覧はスクロール可能にし、ブラックリスト（ワイルドカード対応）で除外可能にする。
14. 表示設定タブの各アプリは表示位置ドロップダウン（1〜10/空）を持ち、同一番号はアプリ間で排他的に割り当てる。
15. 表示制限タブで表示監視モードを切り替え、制限対象アプリを検知した場合はホームへ戻す。
